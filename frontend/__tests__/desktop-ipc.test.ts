...
test.describe('Desktop IPC bridge', () => {
  ...
  test('renderer can call start/pause/stop over preload bridge', async () => {
    ...
    try {
      ...
      const hasDesktopBridge = await window.evaluate(() => typeof window.desktop !== 'undefined');
      expect(hasDesktopBridge).toBe(true);

      const started = await window.evaluate(() => window.desktop?.recording.start());
      expect(started?.isRecording).toBe(true);
      expect(started?.isPaused).toBe(false);

      await window.waitForTimeout(1200);
      const statusAfterTick = await window.evaluate(() => window.desktop?.recording.getStatus());
      expect(statusAfterTick?.elapsedSeconds).toBeGreaterThan(0);

      const paused = await window.evaluate(() => window.desktop?.recording.pauseResume());
      expect(paused?.isRecording).toBe(true);
      expect(paused?.isPaused).toBe(true);

      const resumed = await window.evaluate(() => window.desktop?.recording.pauseResume());
      expect(resumed?.isRecording).toBe(true);
      expect(resumed?.isPaused).toBe(false);

      const stopped = await window.evaluate(() => window.desktop?.recording.stop());
